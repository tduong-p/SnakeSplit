...
// GET /api/boards?status=active|pending|completed
...
// PATCH /api/boards/:id — update name/date (only while active)
...
// POST /api/boards/:id/close — active → pending
...
// POST /api/boards/:id/complete — pending → completed
...
// POST /api/boards/:id/expenses — add expense column
router.post('/:id/expenses', async (req, res) => {
  ...
  if (isSplit && totalAmount && splitAmong?.length) {
    ...
    // Only accept amounts for people in the board (host + participants)
...
// PATCH /api/boards/:id/expenses/:expenseId — edit expense column
...
// DELETE /api/boards/:id/expenses/:expenseId — delete expense column
...
// PATCH /api/boards/:id/payments/:userId — toggle payment status
router.patch('/:id/payments/:userId', async (req, res) => {
  await connectDB();
  const board = await Board.findById(req.params.id);
  if (!board) return res.status(404).json({ error: 'Board not found' });
  if (board.status !== 'pending') return res.status(400).json({ error: 'Board is not pending' });
